({
    ...
    recordUpdateDeal: function(component, event, helper) {
       ...
       if (dealStage == "Term Sheet Signed/Deposit Collected" || dealStage == "Underwriting") 
       {
          component.set("v.ShowValue", "Yes");
       } 
       else
	   {           
        if (profile == "Origination Manager" || profile == "Sales Analyst" || profile == "Relationship Manager"
          || profile == "Sales Analyst Bridge Operations Hybrid" || profile == "Sales Analyst Term Operations Hybrid")
       {
	...
	openReportml: function(component, event, helper) {
		...
		let reportUrl = '/lightning/r/Report/00O76000000JU4YEAW/view?fv0=' + recordId;
		window.open(reportUrl);
	},
    
	openReportsh: function(component, event, helper) {
		let recordId = component.get('v.recordId');
		let reportUrl = '/lightning/r/Report/00O76000000JU4TEAW/view?fv0=' + recordId;
		window.open(reportUrl);
	},
	...
	getHouseCanaryData1: function(component, event, helper) {
        var newCheckBoxValue = component.find("OKtocallValueReport").get("v.checked");
        if (newCheckBoxValue)
        {
        component.set("v.valueReportPreMessage", false);
