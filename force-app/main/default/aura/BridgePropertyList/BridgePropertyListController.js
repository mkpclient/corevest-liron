({
  ...
  handleValueChangeEvent: function (component, event, helper) {
    ...
    const updRow = event.getParam("row");
		const rowIdx = event.getParam("rowIdx");
    const fieldPath = event.getParam("fieldName");
    let rowsChecked = [];
    table.getChecked(function (resp) {
      rowsChecked = resp;
		...
		//need to check because this event gets called for each update
		for(let k = 0; k < rowsChecked.length; k++){
			let i = rowsChecked[k];
			const fields = fieldPath.split('.');
			needUpdating = (fields.length > 1 && rows[i][fields[0]][fields[1]] != updRow[fields[0]][fields[1]]) || (fields.length === 1 && rows[i][fields[0]] != updRow[fields[0]]);
			if(needUpdating) {
				break;
			}
		}
    ...
    if (
			// fieldPath !== component.get("v.lastBulkField") &&
			component.get("v.bulkUpdateFields").includes(fieldPath) &&
      !!updRow &&
      !!updRow.Id &&
			rowsChecked.length > 0 &&
      rowsChecked.includes(rowIdx) &&
			component.get("v.appName") === "Title and Escrow" &&
			needUpdating
    ) {
			console.log(" before confirm ");
        if(window.confirm("Would you like to update all " + rowsChecked.length + " records?" )) {
					const fields = fieldPath.split('.');
					rowsChecked.forEach(i => {
						if(fields.length > 1) {
							rows[i][fields[0]][fields[1]] = updRow[fields[0]][fields[1]];
						} else {
							rows[i][fields[0]] = updRow[fields[0]];
						}
					});
					table.set("v.rows", [].concat(rows));
					component.set("v.lastBulkField", fieldPath);
				}
    ...
    for (var i = 0; i < rowsChecked.length; i++) {
      if (
        ...
        (rows[rowsChecked[i]].Status__c == "Active" &&
          rows[rowsChecked[i]].Reno_Advance_Amount_Remaining__c > 0)
      ) {
  ...
  openRefinanceModal: function (component, event, helper) {
    ...
    if (rowsChecked == "" || rowsChecked == null) {
      ...
      component.set(
        "v.Message",
        "Please select 1 or more Properties before you assign a Refinance Deal."
      );
    } else {
  ...
  onAssignRefinanceDealSave: function (component, event, helper) {
    ...
    var rows = table.get("v.rows");
    for (var i = 0; i < rowsChecked.length; i++) {
      rows[rowsChecked[i]].Deal_Refinanced_With__c = component.get(
        "v.selectedRecord.Id"
      );
    }
