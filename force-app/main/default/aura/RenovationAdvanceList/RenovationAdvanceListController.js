({
  init: function (component, event, helper) {
    ...
    var queryString = `SELECT Id, Advance__c, Advance__r.Status__c, Advance__r.Name, Advance__r.Wire_Date__c, Purchase_Price__c, Initial_Disbursement__c, Renovation_Reserve__c, Approved_Advance_Amount_Calc__c, Property__r.Status__c FROM Property_Advance__c WHERE Property__c = '${recordId}'`;
    queryString += " ORDER BY Wire_Date__c ASC NULLS LAST";
    console.log(queryString);

    component.find("util").query(queryString, (data) => {
      component.set("v.records", data);
      if (data.length > 0) {
        component.set("v.propertyStatus", data[0].Property__r.Status__c);
      }
    });
  ...
  createAdvance: function (component, event, helper) {
    ...
    let recordId = component.get("v.recordId");
    let queryString = `Select Id, Status__c, Reno_Advance_Amount_Remaining__c, Deal__c, Asset_Maturity_Date__c, Current_Asset_Maturity_Date__c, Renovation_Type_formula__c, Deal__r.Max_Advance_Fee__c, Advance_Fee_formula__c, Deal__r.Advance_Fee_Remaining__c FROM Property__c`;
    ...
    component.find("util").query(queryString, (data) => {
      ...
      if (
        ...
        !$A.util.isEmpty(property.Current_Asset_Maturity_Date__c) &&
        new Date(property.Current_Asset_Maturity_Date__c) < new Date()
      ) {
        var toast = $A.get("e.force:showToast");
        toast.setParams({
          type: "warning",
