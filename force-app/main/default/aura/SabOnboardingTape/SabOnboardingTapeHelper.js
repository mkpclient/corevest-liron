({
  retrieveProperties: function (component, helper) {
    ...
    const queryString =
      "SELECT " +
      fields.join(",") +
      " FROM Property__c WHERE (Deal__r.isClosed = false OR Deal__r.StageName IN ('Sold', 'Closed Won')) AND Deal__r.RecordType.DeveloperName='Single_Asset_Bridge_Loan' AND Deal__r.Onboarding_File_Sent_to_Servicer__c = NULL ORDER BY Deal__r.Deal_Loan_Number__c ASC";
