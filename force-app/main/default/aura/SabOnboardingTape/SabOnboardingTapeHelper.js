({
  retrieveProperties: function (component, helper) {
    ...
    const fields = ["Id", "Property__c", "Property__r.Deal__c"];
    ...
    const queryString =
      "SELECT " +
      fields.join(",") +
      " FROM Property_Advance__c WHERE Advance__r.Name = '1' AND (Property__r.Deal__r.isClosed = false OR Property__r.Deal__r.StageName IN ('Sold', 'Closed Won')) AND Property__r.Deal__r.RecordType.DeveloperName='Single_Asset_Bridge_Loan' AND Property__r.Deal__r.Onboarding_File_Sent_to_Servicer__c = NULL ORDER BY Property__r.Deal__r.Deal_Loan_Number__c ASC";
  ...
  saveDealDocument: function (component, helper, fileData, fileName) {
    ...
    props.forEach((p) => {
      if (!dealIds.includes(p.Property__r.Deal__c)) {
        dealIds.push(p.Property__r.Deal__c);
      }
    });
